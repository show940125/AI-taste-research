...
本文處理的問題很具體：大型語言模型輸出的「模板味」，在中文裡到底長成什麼樣子；哪些部分屬於一般生成模型的平均化傾向，哪些又和英譯中殘影糾纏在一起；我們若真想把這股味道壓下去，靠的應該是禁詞、角色扮演，還是重寫流程。研究分成兩段。前半段回顧同質化、translationese、style transfer 與實務 heuristics 四束文獻。後半段用本研究自行建立的第一輪正式結果做初步實測：8 題、24 份正式輸出，條件為 `baseline`、`rewrite-pipeline`、`pivot-pipeline`，writer backend 統一為 `subagent`。結果顯示，高頻姿態詞並非主要問題，真正穩定的訊號落在句法骨架、被動殘影、顯性連接與段落節奏。若以壓低英譯中殘影為主要目標，`pivot-pipeline` 的效果最好；若以保住資訊量為優先，`rewrite-pipeline` 更穩。本文最後提出一套 prompt 模型，重心放在先診斷，再改寫；必要時做跨層中轉，最後用有邊界的 heuristics 做終稿稽核。

**關鍵詞：** AI味、模板味、translationese、英譯中、prompt design、style transfer、中文寫作

## 一、研究緣起

中文使用者談模型寫作，常先從幾個刺眼的地方下手：有些詞太常見，有些句子太整齊，有些段落像從同一個模子倒出來。這些觀察有用，卻還沒碰到問題最硬的那一層。真正讓人一眼看出「這段多半是模型寫的」，往往不在單一詞，也不在一句口頭禪，關鍵在整篇文字落在一個太穩、太平、太工整的位置。它像被磨過，接縫不見了，起伏也不見了。

這件事在中文裡尤其複雜，因為中文 AI 味常和翻譯腔疊在一起。句子表面通順，讀下去卻覺得像英文先在腦子裡排好隊，再被搬到中文。代詞偏滿，邏輯路標偏密，抽象名詞一節接一節，段尾愛收在一個漂亮卻空的判斷上。於是同一篇文本裡，常同時有兩股味道。一股來自生成模型的平均化，一股來自英語思路映到中文。

本研究的目的，就是把這兩股味道拆開看，並把拆解結果做成可用的對抗方案。問題有三個：
...
本文說的「模板味」，指的是輸出在結構上的重複性與可預測性過高。常見表現包括：論證開展太平順、段落功能太平均、句法骨架反覆重現、詞彙落點安全而籠統。這個定義比「高頻詞」大，也比「文筆差」準。它關心的是分布過度集中，變異不足。

### 2.2 AI味

「AI味」是讀者的整體感受。它除了模板味，還包含聊天殘影、討好腔、過度說明、視覺格式痕跡。若把模板味看成骨架，AI味更像整體氣味，骨架只是其中最硬的一層。

### 2.3 英譯中殘影

本文用「英譯中殘影」指模型先在英語化結構上完成規劃，再把內容壓成中文的痕跡。常見位置包括：抽象名詞鏈、顯性關係詞、被動句、代詞過滿、長 `的` 鏈、題面詞重用率偏高。這層和翻譯研究中的 translationese、MTese、third code 密切相連。

### 2.4 研究問題

本文聚焦四個問題：

1. 模板味最穩定的訊號，是否真落在高頻詞。
2. 哪些題型最容易把英譯中殘影放大。
3. `rewrite-pipeline` 與 `pivot-pipeline` 各自修掉什麼，又各自留下什麼。
...
近年的研究對這件事已有相當一致的描述。Zanotto 與 Aroyehun（2025）比較多領域人類文本與多個模型文本，指出模型生成文的變異顯著較低，而且較新的模型彼此更像。這個結果很重要，因為它把「一眼像 AI」從主觀抱怨，往「統計上過度集中」推進了一步。Zamaraeva、Flickinger、Bond 與 Gómez-Rodríguez（2025）則從形式句法的角度補證，發現模型新聞文本與人類新聞文本在句法型別分布上有穩定差異。兩項研究放在一起看，說明模板味並非只是讀者脾氣壞，而是輸出分布真的偏窄。

Durandard、Dhawan 與 Poibeau（2025）更細地指出，模型對題面貼得太近，風格適配卻偏弱。模型常迅速重用問題裡的概念與語詞，回答雖不離題，聲調卻過於 generic。這很接近中文使用者熟悉的感覺：有答案，卻沒有作者。
...
`blader/humanizer` 與維基的「AI生成文的特徵」頁面提供了另一種材料（blader, n.d.; Wikipedia contributors, 2026）。它們不是嚴格的學術證明，卻有極高的編修價值。原因很簡單：它們把大量一線編輯的直覺整理成可操作清單，像姿態膨脹、模糊歸因、否定排比、三段式、聊天善後句、破折號濫用等。

它們最適合放在規則層。也就是說，拿來做標註表、後處理、終稿 audit，非常好用；若要單靠它們判定作者身分，力道就過頭了。這個邊界在研究裡要講明白，否則很容易把 heuristics 用成偽證據。
